#!/bin/bash

CONFIG_DIR="/etc/hysteria"
CONFIG_FILE="$CONFIG_DIR/config.json"
USER_DB="$CONFIG_DIR/udpusers.db"
SYSTEMD_SERVICE="/etc/systemd/system/hysteria-server.service"

mkdir -p "$CONFIG_DIR"
touch "$USER_DB"

fetch_users() {
    if [[ -f "$USER_DB" ]]; then
        sqlite3 "$USER_DB" "SELECT username || ':' || password FROM users;" | paste -sd, -
    fi
}

update_userpass_config() {
    local users=$(fetch_users)
    local user_array=$(echo "$users" | awk -F, '{for(i=1;i<=NF;i++) printf "\"" $i "\"" ((i==NF) ? "" : ",")}')
    jq ".auth.config = [$user_array]" "$CONFIG_FILE" > "${CONFIG_FILE}.tmp" && mv "${CONFIG_FILE}.tmp" "$CONFIG_FILE"
}

add_user() {
    echo -e "\n\e[1;34mEnter username:\e[0m"
    read -r username
    echo -e "\e[1;34mEnter password:\e[0m"
    read -r password
    sqlite3 "$USER_DB" "INSERT INTO users (username, password) VALUES ('$username', '$password');"
    if [[ $? -eq 0 ]]; then
        echo -e "\e[1;32mUser $username added successfully.\e[0m"
        update_userpass_config
        restart_server
    else
        echo -e "\e[1;31mError: Failed to add user $username.\e[0m"
    fi
}

edit_user() {
    echo -e "\n\e[1;34mEnter username to edit:\e[0m"
    read -r username
    echo -e "\e[1;34mEnter new password:\e[0m"
    read -r password
    sqlite3 "$USER_DB" "UPDATE users SET password = '$password' WHERE username = '$username';"
    if [[ $? -eq 0 ]]; then
        echo -e "\e[1;32mUser $username updated successfully.\e[0m"
        update_userpass_config
        restart_server
    else
        echo -e "\e[1;31mError: Failed to update user $username.\e[0m"
    fi
}

delete_user() {
    echo -e "\n\e[1;34mEnter username to delete:\e[0m"
    read -r username
    sqlite3 "$USER_DB" "DELETE FROM users WHERE username = '$username';"
    if [[ $? -eq 0 ]]; then
        echo -e "\e[1;32mUser $username deleted successfully.\e[0m"
        update_userpass_config
        restart_server
    else
        echo -e "\e[1;31mError: Failed to delete user $username.\e[0m"
    fi
}

show_users() {
    echo -e "\n\e[1;34mCurrent users:\e[0m"
    sqlite3 "$USER_DB" "SELECT username FROM users;"
}

change_domain() {
    echo -e "\n\e[1;34mEnter new domain:\e[0m"
    read -r domain
    jq ".server = \"$domain\"" "$CONFIG_FILE" > "${CONFIG_FILE}.tmp" && mv "${CONFIG_FILE}.tmp" "$CONFIG_FILE"
    echo -e "\e[1;32mDomain changed to $domain successfully.\e[0m"
    restart_server
}

change_obfs() {
    echo -e "\n\e[1;34mEnter new obfuscation string:\e[0m"
    read -r obfs
    jq ".obfs = \"$obfs\"" "$CONFIG_FILE" > "${CONFIG_FILE}.tmp" && mv "${CONFIG_FILE}.tmp" "$CONFIG_FILE"
    echo -e "\e[1;32mObfuscation string changed to $obfs successfully.\e[0m"
    restart_server
}

change_up_speed() {
    echo -e "\n\e[1;34mEnter new upload speed (Mbps):\e[0m"
    read -r up_speed
    jq ".up_mbps = $up_speed" "$CONFIG_FILE" > "${CONFIG_FILE}.tmp" && mv "${CONFIG_FILE}.tmp" "$CONFIG_FILE"
    jq ".up = \"$up_speed Mbps\"" "$CONFIG_FILE" > "${CONFIG_FILE}.tmp" && mv "${CONFIG_FILE}.tmp" "$CONFIG_FILE"
    echo -e "\e[1;32mUpload speed changed to $up_speed Mbps successfully.\e[0m"
    restart_server
}

change_down_speed() {
    echo -e "\n\e[1;34mEnter new download speed (Mbps):\e[0m"
    read -r down_speed
    jq ".down_mbps = $down_speed" "$CONFIG_FILE" > "${CONFIG_FILE}.tmp" && mv "${CONFIG_FILE}.tmp" "$CONFIG_FILE"
    jq ".down = \"$down_speed Mbps\"" "$CONFIG_FILE" > "${CONFIG_FILE}.tmp" && mv "${CONFIG_FILE}.tmp" "$CONFIG_FILE"
    echo -e "\e[1;32mDownload speed changed to $down_speed Mbps successfully.\e[0m"
    restart_server
}

show_online_users() {
    echo -e "\n\e[1;34mChecking online users...\e[0m"
    
    # ตรวจสอบว่าเซิร์ฟเวอร์กำลังทำงานอยู่หรือไม่
    if ! pgrep -x "hysteria" > /dev/null; then
        echo -e "\e[1;31mHysteria server is not running.\e[0m"
        return
    fi

    # ดึงพอร์ตจาก config file
    local port=$(jq -r '.listen' "$CONFIG_FILE" | tr -d ':')
    echo -e "\e[1;36mActive connections on port $port:\e[0m"
    
    # ใช้ ss (modern) หรือ netstat (legacy) เพื่อดูการเชื่อมต่อ
    if command -v ss >/dev/null 2>&1; then
        ss -tupn | grep ":$port" | grep -v "LISTEN" | while read line; do
            local state=$(echo $line | awk '{print $1}')
            local remote=$(echo $line | awk '{print $5}')
            local process=$(echo $line | awk '{print $7}')
            echo -e "  \e[1;32m$remote\e[0m - State: \e[1;33m$state\e[0m"
        done
    elif command -v netstat >/dev/null 2>&1; then
        netstat -tupn 2>/dev/null | grep ":$port" | grep -v "LISTEN" | while read line; do
            local proto=$(echo $line | awk '{print $1}')
            local remote=$(echo $line | awk '{print $5}')
            local state=$(echo $line | awk '{print $6}')
            echo -e "  \e[1;32m$remote\e[0m - State: \e[1;33m$state\e[0m"
        done
    else
        echo -e "\e[1;31mCannot check connections: netstat or ss not found.\e[0m"
    fi

    # นับจำนวนการเชื่อมต่อทั้งหมด
    local connection_count=0
    if command -v ss >/dev/null 2>&1; then
        connection_count=$(ss -tupn | grep ":$port" | grep -v "LISTEN" | wc -l)
    elif command -v netstat >/dev/null 2>&1; then
        connection_count=$(netstat -tupn 2>/dev/null | grep ":$port" | grep -v "LISTEN" | wc -l)
    fi
    
    echo -e "\n\e[1;36mTotal active connections: \e[1;32m$connection_count\e[0m"
    
    # ตรวจสอบ log ล่าสุดสำหรับข้อมูลการเชื่อมต่อ
    if journalctl -u hysteria-server.service --no-pager -n 10 2>/dev/null | grep -q "client connected"; then
        echo -e "\n\e[1;36mRecent connections from logs:\e[0m"
        journalctl -u hysteria-server.service --no-pager -n 20 | grep -E "(client connected|auth success)" | tail -5
    fi
}

show_server_stats() {
    echo -e "\n\e[1;34mServer Statistics:\e[0m"
    
    # ตรวจสอบสถานะบริการ
    if systemctl is-active --quiet hysteria-server.service; then
        echo -e "  Service Status: \e[1;32mRUNNING\e[0m"
        
        # แสดงเวลาเริ่มทำงานของบริการ
        local uptime=$(systemctl show hysteria-server.service --property=ActiveEnterTimestamp | awk -F= '{print $2}')
        if [[ -n "$uptime" ]]; then
            echo -e "  Started At: \e[1;33m$uptime\e[0m"
        fi
        
        # แสดงการใช้งาน memory
        local memory=$(ps -o pid,user,%mem,command ax | grep hysteria | grep -v grep | awk '{print $3}')
        if [[ -n "$memory" ]]; then
            echo -e "  Memory Usage: \e[1;33m${memory}%\e[0m"
        fi
    else
        echo -e "  Service Status: \e[1;31mSTOPPED\e[0m"
    fi
    
    # แสดงพอร์ตที่ใช้
    local port=$(jq -r '.listen' "$CONFIG_FILE" | tr -d ':')
    echo -e "  Listening Port: \e[1;33m$port\e[0m"
    
    # แสดงจำนวนผู้ใช้ทั้งหมดในฐานข้อมูล
    local user_count=$(sqlite3 "$USER_DB" "SELECT COUNT(*) FROM users;" 2>/dev/null || echo "0")
    echo -e "  Total Users: \e[1;33m$user_count\e[0m"
    
    # แสดง domain ปัจจุบัน
    local domain=$(jq -r '.server' "$CONFIG_FILE")
    echo -e "  Domain: \e[1;33m$domain\e[0m"
    
    # แสดง obfuscation string
    local obfs=$(jq -r '.obfs' "$CONFIG_FILE")
    echo -e "  Obfuscation: \e[1;33m$obfs\e[0m"
    
    # แสดงความเร็ว
    local up_speed=$(jq -r '.up_mbps' "$CONFIG_FILE")
    local down_speed=$(jq -r '.down_mbps' "$CONFIG_FILE")
    echo -e "  Upload Speed: \e[1;33m${up_speed} Mbps\e[0m"
    echo -e "  Download Speed: \e[1;33m${down_speed} Mbps\e[0m"
}

restart_server() {
    systemctl restart hysteria-server
    echo -e "\e[1;32mServer restarted successfully.\e[0m"
}

uninstall_server() {
    echo -e "\n\e[1;34mUninstalling SHANVPN UDP server...\e[0m"
    systemctl stop hysteria-server
    systemctl disable hysteria-server
    rm -f "$SYSTEMD_SERVICE"
    systemctl daemon-reload
    rm -rf "$CONFIG_DIR"
    rm -f /usr/local/bin/hysteria
    echo -e "\e[1;32mSHANVPN UDP server uninstalled successfully.\e[0m"
}

show_banner() {
    echo -e "\e[1;36m---------------------------------------------"
    echo " SHANVPN UDP Manager"
    echo " (c) 2025 SHANVPN"
    echo " Telegram: @ovpnth"
    echo -e "---------------------------------------------\e[0m"
}

show_menu() {
    echo -e "\e[1;36m----------------------------"
    echo " SHANVPN UDP Manager"
    echo -e "----------------------------\e[0m"
    echo -e "\e[1;32m1. Add new user"
    echo "2. Edit user password"
    echo "3. Delete user"
    echo "4. Show users"
    echo "5. Change domain"
    echo "6. Change obfuscation string"
    echo "7. Change upload speed"
    echo "8. Change download speed"
    echo "9. Restart server"
    echo "10. Show online users"
    echo "11. Show server statistics"
    echo "12. Uninstall server"
    echo -e "13. Exit\e[0m"
    echo -e "\e[1;36m----------------------------"
    echo -e "Enter your choice: \e[0m"
}

show_banner
while true; do
    show_menu
    read -r choice
    case $choice in
        1) add_user ;;
        2) edit_user ;;
        3) delete_user ;;
        4) show_users ;;
        5) change_domain ;;
        6) change_obfs ;;
        7) change_up_speed ;;
        8) change_down_speed ;;
        9) restart_server ;;
        10) show_online_users ;;
        11) show_server_stats ;;
        12) uninstall_server; exit 0 ;;
        13) exit 0 ;;
        *) echo -e "\e[1;31mInvalid choice. Please try again.\e[0m" ;;
    esac
    echo
done